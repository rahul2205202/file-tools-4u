...
import { convertImage } from '@/lib/apiService'; // Using the path alias
            ...
            <div className="bg-white rounded-lg shadow-xl p-8">
                ...
                <form onSubmit={handleSubmit} className="mb-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                        ...
                        <div>
                            <label htmlFor="format-select" className="block text-sm font-bold text-black mb-2">2. Select Output Format</label>
                            <select id="format-select" value={targetFormat} onChange={handleFormatChange} className="w-full p-2.5 border border-black bg-white text-black rounded-md focus:ring-2 focus:ring-black focus:border-black">
                        ...
                        <div>
                             <button type="submit" disabled={isLoading || !selectedFile} className="w-full bg-black text-white font-bold py-2.5 px-4 rounded-md transition-colors duration-300 hover:bg-gray-800 disabled:bg-gray-400 disabled:cursor-not-allowed">
                                {isLoading ? 'Converting...' : '3. Convert Image'}
                            </button>
                        </div>
                    </div>
                </form>
                    ...
                    <div className="border border-gray-200 rounded-lg p-4">
                        <h3 className="text-lg font-bold text-black mb-4">Converted ({targetFormat.toUpperCase()})</h3>
                         <div className="h-80 flex items-center justify-center bg-gray-50 rounded-md">
                           ...
                           {convertedImageUrl && !isLoading && <img src={convertedImageUrl} alt="Converted preview" className="max-h-full max-w-full object-contain" />}
                           {!isLoading && !convertedImageUrl && <p className="text-gray-400">Your converted image will appear here</p>}
                        </div>
                        {convertedImageUrl && <a href={convertedImageUrl} download={getDownloadFileName()} className="mt-4 w-full block text-center bg-green-600 text-white font-bold py-2 px-4 rounded-md transition-colors duration-300 hover:bg-green-700">Download Converted Image</a>}
                    </div>
                </div>
            </div>
            ...
            <div className="mt-12 bg-white rounded-lg shadow-xl p-8">
                ...
                <div className="text-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 max-w-4xl mx-auto text-left">
                        <div className="flex items-start space-x-4"><div className="flex-shrink-0 w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></div><div><h4 className="text-lg font-semibold text-black mb-1">Multiple Formats</h4><p className="text-gray-600">Convert to and from the most popular image formats, including JPEG, PNG, GIF, and BMP.</p></div></div>
                        <div className="flex items-start space-x-4"><div className="flex-shrink-0 w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></div><div><h4 className="text-lg font-semibold text-black mb-1">Instant Previews</h4><p className="text-gray-600">See a preview of your original image and the converted result side-by-side before you download.</p></div></div>
                        <div className="flex items-start space-x-4"><div className="flex-shrink-0 w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path><path d="M12 18.5V18.5" stroke="white" strokeWidth="2" strokeLinecap="round"></path><path d="M12 5.5V5.5" stroke="white" strokeWidth="2" strokeLinecap="round"></path></svg></div><div><h4 className="text-lg font-semibold text-black mb-1">Transparency Handling</h4><p className="text-gray-600">Automatically adds a white background when converting from transparent formats like PNG.</p></div></div>
                        <div className="flex items-start space-x-4"><div className="flex-shrink-0 w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 20.417l5.5-5.5a1 1 0 011.414 0l5.5 5.5A12.02 12.02 0 0012 21.944a11.955 11.955 0 018.618-3.04m-3.04-7.016a1 1 0 011.414 0l2.12 2.12a1 1 0 010 1.414l-2.12 2.12a1 1 0 01-1.414 0l-2.12-2.12a1 1 0 010-1.414l2.12-2.12z"></path></svg></div><div><h4 className="text-lg font-semibold text-black mb-1">Secure & Private</h4><p className="text-gray-600">Your files are processed securely and are never stored on our servers.</p></div></div>
                    </div>
                </div>
            </div>
        </div>
